## Gramatica (resumo)
## programa -> funcdecl | funcdecl programa | comandos | comandos programa
## funcdecl -> signature | signatureproto body
## vardecl -> tipodecl ID : tipo | tipodecl ID : tipo = expressao
##          | tipodecl ID : tipo [ ] = [ ] | tipodecl ID : tipo [ ] = [ listexp ]
##          | tipodecl ID : ( tipo | tipo ) [ ] = [ listexp ]
...
## comando -> vardecl ; | expressao ; | while ( expressao ) bodyorcomando
##          | return expressao ; | if ( expressao ) bodyorcomando
...
##            | NINT | NFLOAT | string | call | assign | ID | TRUE | FALSE
...
def p_vardecl(p):
    ...
    p[0] = sa.varDeclID(p[4], p[2], p[1])

def p_vardecl2(p):
    '''vardecl : tipodecl ID COLON tipo ASSIGN expressao'''
    p[0] = sa.varDeclIDAssign(p[4], p[2], p[6], p[1])

def p_vardecl3(p):
    '''vardecl : tipodecl ID COLON tipo LBRACKET RBRACKET ASSIGN LBRACKET RBRACKET'''
    p[0] = sa.varDeclIDAssignList(p[4], p[2], None, p[1])

def p_vardecl4(p):
    '''vardecl : tipodecl ID COLON tipo LBRACKET RBRACKET ASSIGN LBRACKET listexp RBRACKET'''
    p[0] = sa.varDeclIDAssignList(p[4], p[2], p[9], p[1])

def p_vardecl5(p):
    '''vardecl : tipodecl ID COLON LPAREN tipo BAR tipo RPAREN LBRACKET RBRACKET ASSIGN LBRACKET listexp RBRACKET'''
    p[0] = sa.varDeclIDAssignListTIPO(p[1], p[2], p[5], p[7], p[13])

def p_funcdecl(p):
    '''funcdecl : signature'''
    p[0] = sa.funcDeclSignature(p[1], p[1].body)

def p_funcdecl_prototipo(p):
    '''funcdecl : signatureproto body'''
...
def p_signature1_noparams(p):
    '''signature : tipofunc ID LPAREN RPAREN COLON tipo body'''
    p[0] = sa.signatureFunc(p[1], p[2], None, p[6], p[7])

def p_signature2(p):
    '''signatureproto : tipofunc ID LPAREN funcparametros RPAREN COLON tipo SEMICOLON'''
    p[0] = sa.signatureFunc(p[1], p[2], p[4], p[7], None)

def p_signature2_noparams(p):
    '''signatureproto : tipofunc ID LPAREN RPAREN COLON tipo SEMICOLON'''
    p[0] = sa.signatureFunc(p[1], p[2], None, p[6], None)
...
def p_funcparametros(p):
    '''funcparametros : ID COLON tipo COMMA funcparametros
                 | ID COLON tipo'''
    if len(p) == 6:
        p[0] = sa.funcParametrosIDList(p[3], p[1], p[5])
    else:
        p[0] = sa.funcParametrosID(p[3], p[1])
...
def p_opexp(p):
    '''opexp : expressao
             | VOID
    '''
    if isinstance(p[1], sa.expressao):
        p[0] = sa.ExpOpexp(p[1])
    else:
...
def p_expressaoFIM(p):
    '''expressao : expressao INTER expressao COLON expressao'''
...
def p_expressaoHEXADECIMAL(p):
    '''expressao : HEXADECIMAL'''
    p[0] = sa.ExpressaoInt(p[1])

def p_expressaoOCTAL(p):
    '''expressao : OCTAL'''
    p[0] = sa.ExpressaoInt(p[1])

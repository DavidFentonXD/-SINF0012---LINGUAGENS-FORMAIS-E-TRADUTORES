...
class varDeclID(varDecl):
    # 'tipodecl' (let/var/const) e opcional -- adicionado para a Atividade 6
    # (analise semantica) sem quebrar quem ja construia esses nos so com
    # (tipo, id).
    def __init__(self, tipo, id, tipodecl=None):
        self.tipo = tipo
        self.id = id
...
class varDeclIDAssign(varDecl):
    def __init__(self, tipo, id, expressao, tipodecl=None):
        self.tipo = tipo
        self.id = id
...
class varDeclIDAssignList(varDecl):
    def __init__(self, tipo, id, listexp, tipodecl=None):
        self.tipo = tipo
        self.id = id
...
class varDeclIDAssignListTIPO(varDecl):
    ...
    def accept(self, visitor):
        return visitor.visitVarDeclIDAssignListTIPO(self)
